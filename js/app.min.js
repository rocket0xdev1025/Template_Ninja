(() => {
  ...
  const d = document.querySelector(".hero"),
    h = [
      { element: document.querySelector(".hero__parallax--bg"), factor: 1.2 },
      {
        element: document.querySelector(".hero__parallax--buildings"),
        factor: 1.4,
      },
      { element: document.querySelector(".hero__parallax--logo"), factor: 1.6 },
      { element: document.querySelector(".hero__parallax--cat"), factor: 1.8 },
      {
        element: document.querySelector(".hero__parallax--light"),
        factor: 1.3,
      },
      { element: document.querySelector(".hero__parallax--rain"), factor: 2.2 },
      { element: document.querySelector(".hero__container"), factor: 2.4 },
    ],
    u = 1.2 * d.offsetHeight;
  let m = 0,
    g = !1;
  function f() {
    const t = Math.min(m / u, 1);
    h.forEach(({ element: e, factor: o }) => {
      e.style.transform = `translateY(${(t * u) / o}px)`;
    }),
      (g = !1);
  }
  window.addEventListener("scroll", function () {
    (m = window.scrollY), !g && m <= u && ((g = !0), requestAnimationFrame(f));
  }),
    document.addEventListener("click", async function (t) {
      if (t.target.closest(".copy-ca-btn"))
        try {
          t.target.closest(".copy-ca-btn");
  ...
  const p = document.getElementById("rain-box");
  let w;
  w = window.innerWidth > 767.98 ? 50 : 40;
  for (let t = 0; t < w; t++) {
    const t = document.createElement("div");
    t.classList.add("rain-drop");
    const e = 10 * Math.random() + 8,
      o = 100 * Math.random(),
      a = 20 * Math.random() + 7,
      n = 10 * Math.random() + 5;
    (t.style.height = `${e}vw`),
        ...
        if ("click" === e.type) {
          ...
          if (o.closest("[data-goto]")) {
            const a = o.closest("[data-goto]"),
              ...
              i = a.dataset.gotoTop ? parseInt(a.dataset.gotoTop) : 0;
            if (t.fullpage) {
              const e = document
                  .querySelector(`${n}`)
